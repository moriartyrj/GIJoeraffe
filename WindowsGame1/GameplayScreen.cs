    ...
    public class GameplayScreen : GameScreen
    {
        ...
        public override void Update(GameTime gameTime, bool otherScreenHasFocus, bool coveredByOtherScreen)
        {
            ...
            if (IsActive)
            {
                ...
                if (player.IsAlive == true)
                {
                    ...
                    {
                        player.Position.Y = groundHeight - player.Height;
                    }
                    spriteTexture.UpdateFrame(elapsed);
                    player.Position.X += player.Velocity.X * 128.0f * elapsed;
                    ...
                    if (player.Position.X <= 0.0f + 150)
                    {
                        ...
                        for (int i = 0; i < playerBullets.Count; ++i)
                        {
                            ...
                            playerBullets[i].Position.X = playerBullets[i].Position.X + 1.0f;
                        }
                        ...
                        for (int i = 0; i < alienBullets.Count; ++i)
                        {
                            ...
                            alienBullets[i].Position.X = alienBullets[i].Position.X + 1.0f;
                        }
                        ...
                        for (int i = 0; i < aliens.Count; ++i)
                        {
                            ...
                            aliens[i].Position.X = aliens[i].Position.X + 1.0f;
                        }

                        if (background2.Position > worldBounds.Right)
                        {
                            background2.Position = 0.0f - worldBounds.Right + 1;
                        }
                        else if (background2.Position == 0.0f)
                        {
                            background.Position = 0.0f;
                            background2.Position = worldBounds.Right;
                        }
                    ...
                    if (player.Position.X + 300 >= worldBounds.Right)
                    {
                        ...
                        if (background2.Position < 0.0f - worldBounds.Right + 1)
                        {
                            background2.Position = worldBounds.Right;
                        }
                        else if (background2.Position == 0.0f)
                    ...
                    if (player.Position.Y >= worldBounds.Bottom - groundTexture.Height - player.Height)
                        player.Position = new Vector2(player.Position.X, worldBounds.Bottom - groundTexture.Height - player.Height);
        ...
        void Respawn(float elapsed)
        {
            ...
            if (!player.IsAlive)
            {
                ...
                if (player.RespawnTimer <= 0.0f)
                {
                    ...
                    int left = worldBounds.Width / 2 - playerTexture.Width / 2 - 8;
                    int right = worldBounds.Width / 2 + playerTexture.Width / 2 + 8;
        ...
        void UpdateBullets(float elapsed)
        {
            for (int i = 0; i < playerBullets.Count; ++i)
            {
                ...
                if (playerBullets[i].Position.X > worldBounds.Right)
                {
            ...
            for (int i = 0; i < playerBullets.Count; ++i)
            {
                ...
                for (int a = 0; a < aliens.Count; ++a)
                {
                        ...
                        (playerBullets[i].Position.Y >= aliens[a].Position.Y && playerBullets[i].Position.Y <= aliens[a].Position.Y + aliens[a].Height))
                    {
                        ...
                        if (aliens[a].Texture == badguy_blue)
                            cowDied.Play();
                        else
                            alienDied.Play();
                    }
        ...
        private void SpawnAlien()
        {
            ...
            float duration = worldBounds.Bottom / newAlien.Velocity.Length();
        ...
        private void DrawBullets()
        {
            for (int i = 0; i < playerBullets.Count; ++i)
            {
                if (playerBullets[i].IsAlive)
                    ...
                    {
                        ScreenManager.SpriteBatch.Draw(bulletTexture, playerBullets[i].Position, null, Color.White, playerBullets[i].Rotation, new Vector2(bulletTexture.Height / 2, bulletTexture.Width / 2), 1.0f, SpriteEffects.None, 1.0f);
                    }
        ...
        private void DrawBackground(float elapsedTime)
        {
            ...
            ScreenManager.SpriteBatch.Draw(hillsTexture, new Vector2(background2.Position, worldBounds.Bottom - hillsTexture.Height), tint);
            // Draw the ground
            ScreenManager.SpriteBatch.Draw(groundTexture, new Vector2(background.Position, worldBounds.Bottom - groundTexture.Height), tint);
        ...
        void DrawHud()
        {
            ...
            else
            {
                ...
                DrawString(scoreFont, "LEVEL: " + (((baseLevelKillCount - 5) / 5) + 1).ToString(System.Globalization.CultureInfo.CurrentCulture), new Vector2(6, 340), Color.Yellow);

                text = "HIGH SCORE: " + highScore.ToString(System.Globalization.CultureInfo.CurrentCulture);

                size = scoreFont.MeasureString(text);

                DrawString(scoreFont, text, new Vector2(234 - (int)size.X, 340), Color.Yellow);
            }
        }
